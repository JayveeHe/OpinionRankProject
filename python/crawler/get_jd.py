...
def get_jd_rate(pid, pagenum):
    ...
    for i in range(20):
        ...
        try:
            r = requests.get(
                'http://club.jd.com/productpage/p-{}-s-0-t-3-p-{}.html'.format(pid, pagenum), timeout=1,
                headers=headers)
...
if __name__ == '__main__':
    ...
    aa = time.time()
    getjd(1403302102)
    print(get_jd_title(1403302102))
    jd_result = get_jd_rate_all(1403302102)
    fout = open(get_jd_title(1403302102), 'w')
    total = []
    print len(jd_result)
    count = 0
